...
pub fn routes(state: AppState) -> Router<AppState> {
    Router::new()
        ...
        .route("/forgot-password", post(auth_handler::forgot_password).layer(
            axum::middleware::from_fn_with_state(state.clone(), crate::middlewares::sensitive_auth_rate_limit_middleware)
        ))
        .route("/reset-password", post(auth_handler::reset_password).layer(
            axum::middleware::from_fn_with_state(state.clone(), crate::middlewares::sensitive_auth_rate_limit_middleware)
        ))
